...
<div class="row">
    ...
    <article class="col-md-6 col-lg-3 col-xs-12 mb-3 px-3">
        ...
        @if($articulo->imagen !== null && Storage::has('imgs/' . $articulo->imagen))
            ...
            <div class="card-body">
                <a href="{{ route('articulos.vista', ['id' => $articulo->articulos_id]) }}" class="btn btn-success w-100 fw-bold mb-2">VER MÁS</a>
                <form action="{{ route('cart.addItem') }}" method="POST">
            @csrf
            <input type="hidden" name="articulos_id" value="{{ $articulo->articulos_id }}">
            <button type="submit" class="btn mi-navbar w-100 fw-bold">CONTACTAR</button>
        </form>
            </div>
